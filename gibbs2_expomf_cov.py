...
class ExpoMF(BaseEstimator, TransformerMixin):
    ...
    def _gibbs_sampler(self, Y, YT, pi):
        ...
        for u in range(U): # sample for each user
            A_star = sparse.csr_matrix(np.diag(self.A[u, :]))
            cov = matrix(self.lam_y * self.beta.dot(A_star.dot(self.beta.T)) + self.lam_theta * np.eye(self.n_components)).I
            mean = self.lam_y * cov.dot(self.beta.dot(A_star.dot(Y[u, :].T.toarray())))
            self.theta[:, u] = np.random.multivariate_normal(np.squeeze(np.asarray(mean)), cov)
        ...
        for i in range(I): # sample for each item
            A_star = sparse.csr_matrix(np.diag(self.A[:, i])) 
            cov = matrix(self.lam_y * self.theta.dot(A_star.dot(self.theta.T)) + self.lam_beta * np.eye(self.n_components)).I
            mean = self.lam_y * cov.dot(self.theta.dot(A_star.dot(Y[:, i].toarray())))
            self.beta[:, i] = np.random.multivariate_normal(np.squeeze(np.asarray(mean)), cov)
